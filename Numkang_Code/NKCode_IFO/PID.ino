...
void hall_PID(){
  hall_error = HallValue[1] - HallValue[2];
  PTermHall = (int32_t)hall_error*HKp;
}
...
void MixTable(){
  ...
  //PWM_Command[S1] = 3000 - RC_Command[THR];
  //PWM_Command[S2] = 3000 - RC_Command[THR] + PTermHall;
  OCR5A = 3000 - RC_Command[THR];
  OCR5B = 3000 - RC_Command[THR] + PTermHall;
  if(f.ARM && RC_Command[THR] > 1100 && !f.motorCalibrated) PWMwrite();
  else{ 
    PWMwriteAll(MINCOMMAND);
    OCR5A = 3000 - RC_Command[THR];
    OCR5B = 3000 - RC_Command[THR];
  }
}
#endif
/*
