...
void computeRC() {
  ...
  /*for(chan = 0; chan < RC_CHANS; chan++) {
    if((!f.KINECT_MODE && !f.MOUSE_MODE) || (chan != 1 && chan != 2)){
...
void initPCINT(){  
  DDRB &= ~(_BV(DDB1) | _BV(DDB2) | _BV(DDB3) | _BV(DDB4)); //set PORTB as INPUT
  PORTB |= (_BV(PORTB1) | _BV(PORTB2) | _BV(PORTB3) | _BV(PORTB4)); //pull-up HIGH
  cli();
