...
void acc_getdata2() {
  ...
  /*ACC_ORIENTATION2( ((rawADC[0]<<8) | rawADC[1]) ,
                   ((rawADC[2]<<8) | rawADC[3]) ,
                   ((rawADC[4]<<8) | rawADC[5]) );*/
  ACC_ORIENTATION2( ((rawADC[0]<<8) | rawADC[1])>>3 ,
...
void acc_remove_offset2(){ //MPU6050
  static int32_t acc_sample[3]; 
  uint8_t axis;
  if (calibratingA2>0) {
    for (axis = 0; axis < 3; axis++) {
      if (calibratingA2 == ACC_TO_SAMPLE) acc_sample[axis]=0;    
