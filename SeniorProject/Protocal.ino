...
} //output from Arduino i S C > payloadsize command payload...checksum
...
void serialCom(uint8_t port) { //input i S C < payload command value checksum
  uint8_t c;  
...
void evaluateCommand(uint8_t port) {
  switch(cmdISC[port]) {
    ...
    case ISC_RC_PWM_IN:
      headSerialReply(0,6);
      for(uint8_t i=1; i<=3; i++) serialize16(0,RC_Command[i]);
    break;
    
    case ISC_RC_PWM_OUT:
      headSerialReply(0,4);
      serialize16(0,PWM_Command[ROLL]);
      serialize16(0,PWM_Command[PITCH]);
    break;
